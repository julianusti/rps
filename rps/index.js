...
export const play = (p1, p2) => {
  if (!p1.shape || !strategies[p1.shape]) {
    const errorMsg = `"p1" shape type is wrong. Expected type: { ${Object
      .keys(strategies)
      .filter(s => s !== p1.shape)
      .join(' | ')
    } }`;
  ...
  if (!p2.shape || !strategies[p2.shape]) {
    const errorMsg = `"p2" shape type is wrong. Expected type: { ${SHAPES
      .filter(s => s !== p2.shape)
      .join(' | ')
    } }`;
    throw new TypeError(errorMsg);
...
export const playVsComputer = p1 => {
  if (!p1.shape || !strategies[p1.shape]) {
    const errorMsg = `"p1" shape type is wrong. Expected type: { ${SHAPES
      .filter(s => s !== p1.shape)
      .join(' | ')
    } }`;
...
const getRandomShape = shapes => shapes[Math.floor(Math.random() * shapes.length)];
